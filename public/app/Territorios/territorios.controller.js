...
(function() {
    ...
    function TerritoriosController($scope, $timeout, $mdDialog,  TerritoriosFactory, RegionesFactory, CantonesFactory) {
    	$scope.store = store;
        ...
        function setData (argument) {
            $scope.territorio = {'nombre' : '', 'descripcion' : '', 'region': '', 'canton': ''}
        }
        setData();
        ...
        function getCantones () {
            CantonesFactory.getAll()
                .then(function (response) {
                    ...
                    if(response.length > 0){
                        $scope.selectedCanton = response[0];
                    }
                });
        }
        getCantones();
        ...
        function store () {
            ...
            $scope.territorio.canton_id =  $scope.selectedCanton.id;

            if($scope.emptyData === false){
                TerritoriosFactory.store($scope.territorio)
                    .then(function (response) {
                        if(response === 'true'){//registro guardado exitosamente
                            $scope.registro = true;
        ...
        function modificar (territorio) {
            ...
            territorio.canton_id =  $scope.selectedCanton.id;
